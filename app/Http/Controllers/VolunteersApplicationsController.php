...
{	
    ...
    public function result(Request $request) {
		...
		$activity = 1;
		$application->id_activites = $activity;
		...
		$activity_description = DB::table('tblactivites')->where('id_activites', $activity)->value('activity_description');
		$return_array['activity'] = $activity_description;
		
		$activity_time = DB::table('tblactivites')->where('id_activites', $activity)->value('activity_time');
		$return_array['activity_time'] = $activity_time;
		$application->activity_time = $activity_time;

		$activity_location = DB::table('tblactivites')->where('id_activites', $activity)->value('activity_location');
		$return_array['activity_location'] = $activity_location;
		
		$number_of_vacansion = DB::table('tblactivites')->where('id_activites', $activity)->value('number_of_vacansion');
		DB::update('update tblactivites set number_of_vacansion = ? where id_activites = ?',[$number_of_vacansion - 1, $activity]);
